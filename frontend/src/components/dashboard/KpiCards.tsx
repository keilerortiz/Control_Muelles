...
import type { DashboardSummary } from "../../domain/types/dashboard";

const kpiConfig: Record<string, { label: string; Icon: LucideIcon; suffix?: string }> = {
...
export function KpiCards({ summary }: KpiCardsProps) {
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-6">
      {cards.map((card) => {
        const config = kpiConfig[card.key as keyof typeof kpiConfig];
        if (!config) {
          return null;
        }
