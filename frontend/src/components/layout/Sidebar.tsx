...
export function Sidebar({ isOpen = false, onClose }: SidebarProps) {
  ...
  return (
    <>
      {/* Desktop: sidebar fijo + spacer para mantener layout */}
      <aside
        className={`hidden lg:flex lg:fixed lg:inset-y-0 lg:left-0 lg:z-40 ${baseClasses}`}
      >
        <SidebarContent />
      </aside>
      <div className={`hidden lg:block ${collapsed ? "w-16" : "w-64"}`} aria-hidden="true" />

      {/* Móvil: drawer */}
      <aside
        className={`
          fixed inset-y-0 left-0 z-40 flex transform transition-transform duration-300 ease-in-out
