...
import type { Appointment, AppointmentDetail } from "../../../domain/types/appointments";

type DockAppointment = Appointment & {
  DockName?: string;
  DockId?: number | string;
...
type DockAppointmentDetail = AppointmentDetail & {
  SeniorOperators?: string;
  JuniorOperators?: string;
};
...
function splitOperatorNames(csvValue: unknown): string[] {
...
function normalizeDockLabel(appointment?: DockAppointment | null) {
...
export function DockCard({
  ...
  appointmentDetail?: DockAppointmentDetail | null;
  nowMs: number;
}) {
    ...
    <article
      className={`rounded-2xl border bg-white p-4 shadow-sm transition-colors ${
        compliancePercent > 100
          ? "border-error-400 shadow-lg shadow-error-600/50 ring-2 ring-error-300"
