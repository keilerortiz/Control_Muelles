...
const statusVisualMap = {
  AGENDADA: { icon: Calendar, tone: "bg-sky-100 text-sky-700 ring-sky-200" },
  EN_PATIO: { icon: Warehouse, tone: "bg-amber-100 text-amber-700 ring-amber-200" },
  ENTREGA_DOCUMENTOS: { icon: FileCheck, tone: "bg-indigo-100 text-indigo-700 ring-indigo-200" },
  EN_PROCESO: { icon: Clock, tone: "bg-emerald-100 text-emerald-700 ring-emerald-200" },
  PARA_FIRMAR: { icon: CheckCircle2, tone: "bg-violet-100 text-violet-700 ring-violet-200" },
  FINALIZADO: { icon: CheckCircle2, tone: "bg-teal-100 text-teal-700 ring-teal-200" },
  ATENDIDA: { icon: CheckCircle2, tone: "bg-slate-100 text-slate-700 ring-slate-200" },
  OPERACION_CANCELADA: { icon: AlertTriangle, tone: "bg-rose-100 text-rose-700 ring-rose-200" },
};

function getChangedByLabel(row) {
  return row.ChangedByUserName || row.ChangedByName || row.ChangedByUserId || "-";
}
...
function getDiffLabel(currentDate, previousDate) {
...
function DetailField({ label, value, icon: Icon }) {
  return (
    <div className="space-y-0.5">
      ...
      <p className="text-[12px] leading-tight text-slate-800">{value ?? "-"}</p>
    </div>
  );
}
...
export function AppointmentDetailPanel({
  ...
  const renderActions = showActions && (
    <Card title="Acciones">
      {actions.length === 0 ? (
        <p className="text-sm text-slate-500">No hay acciones disponibles.</p>
      ) : (
                ...
                {pagedStatusLog.length === 0 ? (
                  ...
                  historyVariant === "horizontal-compact" ? (
                    <ol className="flex w-full items-start justify-between gap-1">
                      {timelineSequence.map((statusKey, idx) => {
                        ...
                        const visual = statusVisualMap[statusKey] || { icon: Circle, tone: "bg-neutral-100 text-neutral-700 ring-neutral-200" };
                        ...
                        const previousDate =
                          statusIndexInLog > 0 && statusLog[statusIndexInLog - 1]?.ChangedAt
                            ? new Date(statusLog[statusIndexInLog - 1].ChangedAt)
                            : null;
                        const elapsedLabel = getDiffLabel(currentDate, previousDate);
                        const nodeTone = isPending ? "bg-neutral-50 text-neutral-400 ring-neutral-200" : visual.tone;
                        return (
                          <li key={`${statusKey}-${idx}`} className="relative min-w-0 flex-1 last:flex-none">
                            ...
                            <div className="mt-0.5 min-w-0">
                              ...
                              {elapsedLabel ? <p className="truncate text-[9px] font-medium leading-tight text-amber-600">+{elapsedLabel}</p> : null}
                  ...
                  <ol className="space-y-0">
                    {pagedStatusLog.map((row, idx) => {
                      ...
                      const visual = statusVisualMap[row.NewStatus] || { icon: Circle, tone: "bg-neutral-100 text-neutral-700 ring-neutral-200" };
                      const Icon = visual.icon;
                      const currentDate = row.ChangedAt ? new Date(row.ChangedAt) : null;
                      const previousDate = globalIndex > 0 && statusLog[globalIndex - 1]?.ChangedAt ? new Date(statusLog[globalIndex - 1].ChangedAt) : null;
                      const elapsedLabel = getDiffLabel(currentDate, previousDate);
                      return (
                        <li key={row.Id || idx} className="relative pl-10 pb-4 last:pb-0">
                          {idx < pagedStatusLog.length - 1 ? (
                            <span className="absolute left-[17px] top-8 h-[calc(100%-20px)] w-px bg-neutral-200" />
                          ) : null}
                          <span className={`absolute left-0 top-0 inline-flex h-9 w-9 items-center justify-center rounded-full ring-2 ${visual.tone}`}>
