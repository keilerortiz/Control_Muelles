...
interface NonComplianceReasonSelectProps {
  label: string;
  placeholder: string;
  options: string[];
...
export function NonComplianceReasonSelect({
  label,
  placeholder,
  options,
  selectedValues,
  onToggle,
  required = false,
}: NonComplianceReasonSelectProps) {
        ...
        <div className="space-y-2">
          ...
          <div className="max-h-56 space-y-1 overflow-y-auto pr-1">
            {filteredOptions.length === 0 ? (
              <p className="px-2 py-2 text-sm text-neutral-500">No hay resultados para la búsqueda.</p>
            ) : (
              filteredOptions.map((option) => {
                ...
                return (
                  <button
                    ...
                    className="flex w-full items-center gap-2 rounded-lg px-2 py-1.5 text-left text-sm text-neutral-700 hover:bg-neutral-50"
                  >
                    <span className={`inline-flex h-4 w-4 items-center justify-center rounded border ${checked ? "border-brand-500 bg-brand-500 text-white" : "border-neutral-300 bg-white text-transparent"}`}>
