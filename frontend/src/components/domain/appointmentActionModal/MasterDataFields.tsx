...
interface MasterDataFieldsProps {
  ...
  updateValue: (field: keyof MasterFormState, value: string) => void;
}

type SelectOption = { value: number | string; label: string };
type CatalogOptionSource = CatalogItem & { Id: number | string; Name: string; IsActive: boolean };

function isSelectOption(item: SelectOption | undefined): item is SelectOption {
  return Boolean(item?.label && item.value !== undefined && item.value !== null);
}

function isCatalogOptionSource(item: CatalogItem | undefined): item is CatalogOptionSource {
  return Boolean(item?.IsActive && item.Id !== undefined && item.Name);
}

export function MasterDataFields({ form, updateValue }: MasterDataFieldsProps) {
  const catalogsQuery = useMasterCatalogs();
  const clientsRaw = (catalogsQuery.data?.clients || []) as CatalogItem[];
  const operationTypesRaw = (catalogsQuery.data?.operationTypes || []) as CatalogItem[];
  const vehicleTypesRaw = (catalogsQuery.data?.vehicleTypes || []) as CatalogItem[];
  const businessRulesRaw = (catalogsQuery.data?.businessRules || []) as CatalogItem[];

  const clients = clientsRaw
    .filter((item) => item.IsActive)
    .map((item) => ({ value: item.Id, label: item.Name }));
  const operationTypesById = useMemo(
    () => new Map(operationTypesRaw.map((item) => [Number(item.Id), item])),
    [operationTypesRaw],
  );
  const vehicleTypesById = useMemo(
    () => new Map(vehicleTypesRaw.map((item) => [Number(item.Id), item])),
    [vehicleTypesRaw],
  );
  const activeRules = businessRulesRaw.filter((rule) => rule.IsActive);
