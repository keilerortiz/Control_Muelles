...
import { getQueryErrorMessage } from "../domain/appointmentsConfig";
...
export function DashboardPage() {
  ...
  if (summaryQuery.isError || appointmentsQuery.isError) {
    return (
      <ErrorState
        message={getQueryErrorMessage(summaryQuery.error ?? appointmentsQuery.error, "No se pudo cargar el dashboard")}
        onRetry={() => window.location.reload()}
      />
    );
  }

  const appointments = (appointmentsQuery.data?.items || []) as Appointment[];
  const alerts = (summaryQuery.data?.alerts || []) as DashboardAlert[];
