...
class AppointmentRepository:
    ...
    async def create_appointment(self, payload: dict, user_id: int) -> int:
        output_sql = text(
            """
            DECLARE @newId INT;
    ...
    async def update_appointment(self, appointment_id: int, payload: dict, user_id: int) -> None:
        await self.session.execute(
            text(
                "EXEC dbo.usp_UpdateAppointment :appointment_id, :client_id, :operation_type_id, :vehicle_type_id, :estimated_tons, :scheduled_at, :updated_by"
            ),
            {
    ...
    async def delete_appointment(self, appointment_id: int, user_id: int) -> None:
        await self.session.execute(
            text("EXEC dbo.usp_DeleteAppointment :appointment_id, :deleted_by"),
            {"appointment_id": appointment_id, "deleted_by": user_id},
        )

    async def checkin(self, appointment_id: int, payload: dict, user_id: int) -> None:
        await self.session.execute(
            text("EXEC dbo.usp_CheckInAppointment :appointment_id, :driver_name, :driver_document, :vehicle_plate, :changed_by"),
            {
                "appointment_id": appointment_id,
    ...
    async def assign(self, appointment_id: int, dock_id: int, operator_ids: list[int], user_id: int) -> None:
        operators_csv = ",".join(str(operator_id) for operator_id in operator_ids)
        await self.session.execute(
            text("EXEC dbo.usp_AssignAppointmentResource :appointment_id, :dock_id, :operator_ids, :changed_by"),
            {
                "appointment_id": appointment_id,
                "dock_id": dock_id,
    ...
    async def reassign(self, appointment_id: int, dock_id: int, operator_ids: list[int], user_id: int) -> None:
        operators_csv = ",".join(str(operator_id) for operator_id in operator_ids)
        await self.session.execute(
            text("EXEC dbo.usp_ReassignAppointmentResource :appointment_id, :dock_id, :operator_ids, :changed_by"),
            {
                "appointment_id": appointment_id,
                "dock_id": dock_id,
                "operator_ids": operators_csv,
    ...
    async def start_process(self, appointment_id: int, process_start_at: datetime, user_id: int) -> None:
        await self.session.execute(
            text("EXEC dbo.usp_StartAppointmentProcess :appointment_id, :process_start_at, :changed_by"),
            {
                "appointment_id": appointment_id,
    ...
    async def to_sign(self, appointment_id: int, process_end_at: datetime, user_id: int) -> None:
        await self.session.execute(
            text("EXEC dbo.usp_MoveAppointmentToSign :appointment_id, :process_end_at, :changed_by"),
            {
                "appointment_id": appointment_id,
    ...
    async def finalize(self, appointment_id: int, payload: dict, user_id: int) -> None:
        await self.session.execute(
            text(
                "EXEC dbo.usp_FinalizeAppointment :appointment_id, :finalized_at, :moved_weight_kg, :otc_reason, :ots_reason, :comment, :changed_by"
            ),
            {
                "appointment_id": appointment_id,
    ...
    async def checkout(self, appointment_id: int, checkout_at: datetime, user_id: int) -> None:
        await self.session.execute(
            text("EXEC dbo.usp_CheckoutAppointment :appointment_id, :checkout_at, :changed_by"),
            {
                "appointment_id": appointment_id,
    ...
    async def cancel(self, appointment_id: int, cancellation_reason: str, user_id: int) -> None:
        await self.session.execute(
            text("EXEC dbo.usp_CancelAppointment :appointment_id, :reason, :changed_by"),
            {
                "appointment_id": appointment_id,
