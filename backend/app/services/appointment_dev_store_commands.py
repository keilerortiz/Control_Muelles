...
class AppointmentDevStoreCommandsMixin:
    def _validate_scheduled_at(self, scheduled_at: datetime) -> None:
        ...
        if normalized_scheduled <= now:
            raise AppError("La fecha programada debe ser futura", error_code="INVALID_DATE", status_code=400)
    ...
    def create(self, payload: dict[str, Any], user_id: int) -> dict[str, Any]:
        ...
        self._next_id += 1
        item = {"Id": appointment_id, "ClientId": payload["clientId"], "ClientName": f"Cliente {payload['clientId']}", "OperationTypeId": payload["operationTypeId"], "OperationTypeName": "Descargue", "VehicleTypeId": payload["vehicleTypeId"], "VehicleTypeName": "Camion Sencillo", "StandardTimeMinutes": 120, "DriverName": payload.get("driverName"), "DriverDocument": payload.get("driverDocument"), "VehiclePlate": payload.get("vehiclePlate"), "EstimatedTons": payload["estimatedTons"], "DockId": None, "DockName": None, "Status": "AGENDADA", "ScheduledAt": to_iso(payload["scheduledAt"]), "ArrivalAt": None, "DocumentDeliveryAt": None, "ProcessStartAt": None, "ProcessEndAt": None, "FinalizedAt": None, "CheckoutAt": None, "CancelledAt": None, "Remissions": None, "Precincts": None, "MovedWeightKg": None, "NonComplianceComment": payload.get("nonComplianceComment"), "OtcNonComplianceReason": None, "OtsNonComplianceReason": None, "CancellationReason": None, "IsNoShow": False, "IsLateArrival": False, "LateArrivalMinutes": None, "Version": 1, "CreatedAt": to_iso(now), "UpdatedAt": to_iso(now)}
    ...
    def update(self, appointment_id: int, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        if item["Status"] != "AGENDADA":
            raise AppError("Estado invalido para actualizar", error_code="INVALID_STATUS_FOR_UPDATE", status_code=409)
        self._validate_configuration(payload)
        self._validate_scheduled_at(payload["scheduledAt"])
    ...
    def checkin(self, appointment_id: int, payload: dict[str, Any], user_id: int) -> dict[str, Any]:
        item = self._get(appointment_id)
        if item["Status"] != "AGENDADA":
            raise AppError("Transicion invalida", error_code="INVALID_STATE_TRANSITION", status_code=409)
    ...
    def start_process(self, appointment_id: int, document_delivery_at: datetime, process_start_at: datetime, user_id: int) -> dict[str, Any]:
        item = self._get(appointment_id)
        ...
        if normalized_process_start < normalized_document_delivery:
            raise AppError("Inicio de proceso invalido", error_code="INVALID_PROCESS_START_AT", status_code=409)
