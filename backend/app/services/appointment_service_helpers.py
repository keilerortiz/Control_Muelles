...
class AppointmentServiceHelpersMixin:
        ...
        for code, (friendly_message, status_code) in ERROR_METADATA.items():
            if code in message:
                ...
                if "|" in message:
                    ...
                    if suffix:
                        details["rule"] = suffix
                return AppError(message=friendly_message, error_code=code, status_code=status_code, details=details)
    ...
    def _is_db_unavailable(self, exc: DBAPIError) -> bool:
        ...
        patterns = ["08001", "OperationalError", "Client unable to establish connection", "server is not found or not accessible", "Connection refused", "Invalid connection string attribute", "SSL Provider", "No hay credenciales disponibles"]
    ...
    async def _broadcast_change(self, action: str, appointment_id: int, status: str | None, correlation_id: str) -> None:
        payload: dict[str, Any] = {"action": action, "appointmentId": appointment_id, "correlationId": correlation_id}
