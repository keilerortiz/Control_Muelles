...
class AppointmentServiceActionsMixin:
    async def checkin(
        ...
        return await self._run_state_change(
            "CHECKIN",
            appointment_id,
            correlation_id,
            origin_client_id,
            lambda: self.repository.checkin(appointment_id, payload, user_id, correlation_id),
            lambda: DEV_APPOINTMENTS_STORE.checkin(appointment_id, payload, user_id),
        )
    ...
    async def reassign(
        ...
        return await self._run_state_change(
            ...
            correlation_id,
            origin_client_id,
            lambda: self.repository.reassign(
                appointment_id,
                payload["dockId"],
                operator_ids,
                payload["candidatesVersion"],
    ...
    async def start_process(
        ...
        return await self._run_state_change(
            ...
            correlation_id,
            origin_client_id,
            lambda: self.repository.start_process(
                appointment_id,
                now,
                now,
                payload["remissions"],
    ...
    async def finalize(
        ...
        effective_payload = {**payload, "finalizedAt": datetime.now(UTC)}
        return await self._run_state_change(
            "FINALIZE",
            appointment_id,
            correlation_id,
            origin_client_id,
            lambda: self.repository.finalize(
                appointment_id, effective_payload, user_id, correlation_id
            ),
            lambda: DEV_APPOINTMENTS_STORE.finalize(appointment_id, effective_payload, user_id),
        )
    ...
    async def _run_state_change(
        ...
        await self._broadcast_change(
            action, data["Id"], data["Status"], correlation_id, origin_client_id
        )
        return data
